...
MET_pd <- read.csv("../DNAmethylation/sample_Sheet_Linkoping2020.csv",sep=";")
MET_pd$SampleName
...
save(rgSet, file = "rgSet_MS_30052022.RData")
save(samples_info,file="samples_info_MS_30052022.RData")

 
write.table(samples_info,"samples_info_MS_Sputum_25042022.csv",sep=",",quote = F,row.names = F)
head(pData(rgSet))
names(samples_info)
samples_info
...
{ #-------------- Quality Control of the dataset
  ...
  name<-paste0(mycell,"_Raw_Density_1.png")
...
##---------- save the R objects
  save(bVals,file="MS_bVals_30052022.RData")
  save(mVals,file="MS_mVals_30052022.RData")
...
SVDplot.champ<-function(bVals,samples)
  ...
  save(bVals.corrected,file="MS_bVals.corrected_30052022.RData")
...
dev.off()
getwd()
setwd("/Users/lovka39/Documents/Linkoping2021/Results/DNAmethylation/Mouth_swab_final_220524 /Work_30052022/")
###############################################################################
...
DMC_identifcation<-function()
{
  ...
  getwd()
  save(Patient_Control,file="Patient_Control.MouthSwab.RData")
  save(Exposed_Control,file="Exposed_Control.MouthSwab.RData")
  save(Patient_Exposed,file="Patient_Exposed.MouthSwab.RData")
  #
...
## Using the DMCs identified above, you can merge them or use them individually
...
Dotplot<-function()
{
  ...
  # Install and load the package (already installed in the Docker version)
...
CHeatmapPlot<-function(betaPCA,tp1,samples,title1){

  library(ComplexHeatmap)
  library(dendsort)
  library("RColorBrewer")
  top_betaPCA<-betaPCA[rownames(betaPCA) %in% tp1$Name,]
  dim(samples)
  dim(top_betaPCA)
  ## load data end
  match(colnames(top_betaPCA),samples$SampleName)
  data1_top <- as.matrix(top_betaPCA) ##*******************
  
  betaPCAt<-as.data.frame(t(data1_top))

  betaPCAt$group <- factor(betaPCAt$group, levels = c("ATB", "CT","HC"))


  ha <- HeatmapAnnotation(
    group= betaPCAt$group,
    col = col1,

    annotation_label = gt_render(c("Group"),
                                 gp = gpar(col = "black", fontsize=16)),
    annotation_name_side = "right",
    simple_anno_size = ,

    #Sample_Group=anno_text(gp = gpar(fontsize = 22)),
  ...
  png("Heatmap.png",height = 10, width = 10, units = "in", res = 300)
  row_dend = dendsort(hclust(dist(data1_top)))
  col_dend = dendsort(hclust(dist(t(data1_top))))

  Heatmap(data1_top,top_annotation = ha, name = "Beta value", cluster_rows = row_dend, cluster_columns = col_dend,
          column_title = "",row_dend_reorder = TRUE,show_row_names=FALSE,show_column_names =FALSE,show_row_dend = FALSE)# ,col = viridis(100))

  ht=Heatmap(data1_top,top_annotation = ha,
             name = "Beta value",
             ...
             heatmap_legend_param = list(
               title = "Beta value", at = c(0, 0.5,1),
               labels_gp = gpar(col = "black", fontsize = 18),
               title_gp = gpar(col = "black", fontsize = 18)

             ),
             #width = unit(8, "cm"), height = unit(8, "cm"),
             column_title = "",row_dend_reorder = TRUE,show_row_names=FALSE,
             ##--- *** --- ##
